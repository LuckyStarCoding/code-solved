...
# Mastering the Merge Intervals Pattern
...
The most critical step—and the one most often forgotten under interview pressure—is **sorting**. You cannot efficiently merge intervals if they are scattered.
...
2.  **Step 2:** The loop moves to the next index. You compare `[2, 6]` and `[8, 10]`. They don’t overlap, so you push `[2, 6]` into your result.
...
Always double-check the problem constraints—if the prompt specifically says intervals are "non-overlapping" when they touch at a single point, only then would you use the strictly less than operator.
