...
```javascript
// ❌ DON'T DO THIS: Vulnerable to backtick/template breakout
// const result = eval(`${userCode}`); 

// ✅ DO THIS: Securely escapes all special characters
...
To ensure variables from one "Run" don't leak into the next and to prevent **global scope pollution**—which could overwrite your internal sandbox helpers or break the editor's UI—the user logic must be encapsulated.
...
const logToScreen = (msg) => {
  ...
  // ✅ textContent neutralizes HTML/Script payloads automatically
